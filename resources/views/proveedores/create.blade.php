            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border border-gray-100">
                <form method="POST" action="{{ route('proveedores.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="nombre" class="block mt-1 w-full" type="text" name="nombre" :value="old('nombre')" placeholder="Ej. Distribuidora del Valle..." required autofocus />
                        <x-input-error :messages="$errors->get('nombre')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="contacto" :value="__('Persona de Contacto')" />
                        <x-text-input id="contacto" class="block mt-1 w-full" type="text" name="contacto" :value="old('contacto')" placeholder="Nombre de asesor de ventas..." />
