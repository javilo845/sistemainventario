                ...
                <form method="POST" action="{{ route('items.update', $item) }}" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <x-text-input id="nombre" class="block mt-1 w-full" type="text" name="nombre" :value="old('nombre', $item->nombre)" required autofocus />
                            <x-input-error :messages="$errors->get('nombre')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="sku" :value="__('Código / SKU')" />
                            <x-text-input id="sku" class="block mt-1 w-full font-mono uppercase" type="text" name="sku" :value="old('sku', $item->sku)" required />
                        ...
                        <div>
                            <x-input-label for="proveedor_id" :value="__('Proveedor Habitual')" />
                        ...
                        <div>
                            ...
                            <x-text-input id="costo_unitario" class="block mt-1 w-full" type="number" step="0.01" min="0" name="costo_unitario" :value="old('costo_unitario', $item->costo_unitario)" />
                            <x-input-error :messages="$errors->get('costo_unitario')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="stock_minimo" :value="__('Stock Mínimo')" />
                            <x-text-input id="stock_minimo" class="block mt-1 w-full" type="number" min="0" name="stock_minimo" :value="old('stock_minimo', $item->stock_minimo)" required />
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="descripcion" name="descripcion" rows="3" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">{{ old('descripcion', $item->descripcion) }}</textarea>
                            <x-input-error :messages="$errors->get('descripcion')" class="mt-2" />
                        </div>

                        <div class="md:col-span-2">
                            <x-input-label for="imagen" :value="__('Imagen del Ítem (dejar en blanco para conservar)')" />
                            @if($item->imagen)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $item->imagen) }}" alt="Foto" class="w-16 h-16 rounded object-cover border">
                                </div>
                            @endif
